...
static void vec_push_u32(std::vector<unsigned char>& v, uint32_t n) {
...
static void vec_push_u64(std::vector<unsigned char>& v, uint64_t n) {
    unsigned char b[8];
...
static uint32_t vec_read_u32(const std::vector<unsigned char>& v, size_t& pos) {
...
static uint64_t vec_read_u64(const std::vector<unsigned char>& v, size_t& pos) {
    uint64_t n = 0;
...
static std::vector<unsigned char> pack_folder(const std::string& folder_path) {
    ...
    for (const auto& entry : fs::recursive_directory_iterator(root)) {
        ...
        std::vector<unsigned char> content(std::istreambuf_iterator<char>(fin), {});
...
static void unpack_folder(const std::vector<unsigned char>& archive,
