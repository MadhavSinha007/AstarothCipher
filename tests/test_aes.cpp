...
int main() {
    ...
    std::cout << "\n[4] Additional Authenticated Data (AAD)...\n";
    SecureVector aad_val = {'m', 'e', 't', 'a'};
    auto aad_iv = AESGCMCipher::generate_iv();
    SecureVector aad_tag;
    ...
    SecureVector short_key(16, 0xAA); // 128-bit, not 256
    SecureVector dummy_tag;
    ...
    SecureVector short_iv(8, 0xBB); // 64-bit, not 96
